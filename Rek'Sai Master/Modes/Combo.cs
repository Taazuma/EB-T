        ...
        public static void Execute()
        {
            ...
            if (ComboMenu["Comba"].Cast<ComboBox>().CurrentValue == 0)
            {
                if (Program.IsBurrowed())
                {
                    ...
                    {
                        if (E.IsReady() && te.IsValidTarget(E.Range + W.Range) && Program._player.Distance(te) > Q.Range && E2.GetPrediction(targetE2).HitChance >= Hitch.hitchance(E2, FirstMenu))
                        {
                            E2.Cast(te.Position);
                        }
                    }

                    if (ComboMenu.GetCheckBoxValue("UseQBCombo") && Q2.IsReady() && Q2.GetPrediction(targetQ2).HitChance >= Hitch.hitchance(Q2, FirstMenu))
                    {
            ...
            if (ComboMenu["Comba"].Cast<ComboBox>().CurrentValue == 2)
            {
                ...
                // q2 cast
                if (Program.IsBurrowed() && Q2.IsReady() && Q2.GetPrediction(targetQ2).HitChance >= Hitch.hitchance(Q2, FirstMenu))
                {
                ...
                {
                    if (Player.Instance.CountEnemiesInRange(420) >= 0)
                    {
